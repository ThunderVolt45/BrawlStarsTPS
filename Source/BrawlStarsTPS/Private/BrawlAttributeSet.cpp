...
void UBrawlAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if (Data.EvaluatedData.Attribute == GetHealthAttribute())
	{
		...
		if (SourceASC)
		{
			...
			// 값을 찾지 못할 경우 위에서 초기화한 값인 0.0f가 된다
		...
		{
			SourceASC->ApplyModToAttributeUnsafe(GetSuperChargeAttribute(),
				EGameplayModOp::Additive, ChargeAmount);
		}

		if (HyperChargeAmount > 0.0f)
		{
			SourceASC->ApplyModToAttributeUnsafe(GetHyperChargeAttribute(),
				EGameplayModOp::Additive, HyperChargeAmount);
		}
		...
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Cyan, 
				FString::Printf(TEXT("Charge! Super +%.00f, Hyper +%.00f"),
